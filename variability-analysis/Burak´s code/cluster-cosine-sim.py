...
fig_save_path = os.path.join(main_path,"burak_figures","cosine_similarity")
...
linkage_matrix = hierarchy.linkage(cosine_sim, method='ward')  # You can choose different linkage methods
...
reordered_sim = cosine_similarity(reordered_array)
...
ax_dendrogram_cs = plt.subplot(gs[0, :-1])
ax_dendrogram_cs.spines['top'].set_visible(False)
ax_dendrogram_cs.spines['right'].set_visible(False)
ax_dendrogram_cs.spines['bottom'].set_visible(False)
ax_dendrogram_cs.spines['left'].set_visible(False)
ax_dendrogram_cs.get_xaxis().set_visible(False)
ax_dendrogram_cs.get_yaxis().set_visible(False)
hierarchy.dendrogram(linkage_matrix, ax=ax_dendrogram_cs, color_threshold=0)

# Plot the heatmap using the reordered DataFrame
ax_heatmap = plt.subplot(gs[1, :-1])
sns.heatmap(reordered_sim, cmap='rocket', annot=False, xticklabels=cluster_df.index, yticklabels=cluster_df.index, ax=ax_heatmap, cbar=False)
...
for k in range(2, max_c):
    ...
    silhouette_coefficients.append(silhouette_score(cosine_sim, cluster_labels))
...
# %% Clustering 
selected_cluster_n = 8
clusterer = AgglomerativeClustering(n_clusters=selected_cluster_n, linkage='ward')
cluster_labels = clusterer.fit_predict(cosine_sim)

unique_clusters , counts = np.unique(cluster_labels, return_counts= True)

fig4, axs = plt.subplots(len(unique_clusters),figsize=(6, 14),sharey=True)
major_inputs_data = data_df[['L3','Mi4','CT1','Tm16','Dm12']]
for cluster in unique_clusters:
    sns.boxplot(data=major_inputs_data.iloc[np.where(cluster_labels == cluster)[0]],ax = axs[cluster])
    # axs[cluster].set_ylabel('relative counts')
fig4.suptitle(f'Neurons per cluster: {counts}')
...
fig3, axs3 = plt.subplots(selected_cluster_n,figsize=(18, 8))
...
# %% Save figures
fig.savefig(os.path.join(fig_save_path,f'HeatMapHamming_{selected_cluster_n}clusters_{current_data}_excludeMajor-{exclude_neurons}_scale-{scale_data}.pdf'))
fig1.savefig(os.path.join(fig_save_path,f'Silhouette_{selected_cluster_n}clusters_{current_data}_excludeMajor-{exclude_neurons}_scale-{scale_data}.pdf'))
fig2.savefig(os.path.join(fig_save_path,f'BinaryHeatMap_{selected_cluster_n}clusters_{current_data}_excludeMajor-{exclude_neurons}_scale-{scale_data}.pdf'))
fig3.savefig(os.path.join(fig_save_path,f'InputPercentage_{selected_cluster_n}clusters_{current_data}_excludeMajor-{exclude_neurons}_scale-{scale_data}.pdf'))
fig4.savefig(os.path.join(fig_save_path,f'Counts_{selected_cluster_n}clusters_{current_data}_excludeMajor-{exclude_neurons}_scale-{scale_data}.pdf'))
...
for icluster, cluster_n in enumerate(unique_clusters):
    ...
    fig.savefig(os.path.join(fig_save_path,f'Location_{selected_cluster_n}clusters_{current_data}_excludeMajor-{exclude_neurons}_scale-{scale_data}_cluster{cluster_n}_OL-L.pdf'))
...
for icluster, cluster_n in enumerate(unique_clusters):
    ...
    fig.savefig(os.path.join(fig_save_path,f'Location_{selected_cluster_n}clusters_{current_data}__excludeMajor-{exclude_neurons}_scale-{scale_data}_cluster{cluster_n}_OL-R.pdf'))
